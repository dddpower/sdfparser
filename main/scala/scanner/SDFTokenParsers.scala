...
trait SDFTokenParsers extends TokenParsers {
  ...
  def TK_Int: Parser[IntType] = accept(SDF_Keyword("int")) ^^ {
    case kw: SDF_Keyword => IntType()
  }
  def TK_Float: Parser[FloatType] = accept(SDF_Keyword("float")) ^^ {
    case kw: SDF_Keyword => FloatType()
  }
  def TK_Boolean: Parser[BooleanType] = accept(SDF_Keyword("boolean")) ^^ {
    case kw: SDF_Keyword => BooleanType()
  }
  def TK_Complex: Parser[ComplexType] = accept(SDF_Keyword("complex")) ^^ {
    case kw: SDF_Keyword => ComplexType()
  }
  def TK_Bit: Parser[BitType] = accept(SDF_Keyword("bit")) ^^ {
    case kw: SDF_Keyword => BitType()
  }
  def TK_Void: Parser[VoidType] = accept(SDF_Keyword("void")) ^^ {
    case kw: SDF_Keyword => VoidType()
  }
  def tmpID: Parser[ID] = {
